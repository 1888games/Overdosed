...
public class PatientControllerMac : MonoBehaviourSingleton<PatientControllerMac> {
	...
	public float pulseRateReturnToNormal = 0.3f;

	public float pulseRateMaxBeforeCardiacArrest = 120f;
	public float timeInCardiacArrestUntilDead = 25f;

	public float maximumPulseRateIncreaseFromOverdose = 80f;
	...
	void Start () {
		...
		maximumWrongMedEffects.Add ("Pain", 45f);
		maximumWrongMedEffects.Add ("Infection", 55f);
		maximumWrongMedEffects.Add ("Blood", 65f);
		maximumWrongMedEffects.Add ("Defib", 75f);

		minimumWrongMedEffects.Add ("Pain", 25f);
		minimumWrongMedEffects.Add ("Infection", 30f);
		minimumWrongMedEffects.Add ("Blood", 35f);
		minimumWrongMedEffects.Add ("Defib", 45f);
	...
	Medication CheckWhetherToStartRecovery (Medication m) {
	...
	Patient CheckWhetherFullyRecovered (Patient p, bool stillRequiringTreatment) {
		...
		if (stillRequiringTreatment == false) {
	...
	void UpdatePatientStatus(Patient p) {
		...
		for (int i = 0; i < p.currentPrescriptions.Count; i++) {
			...
			m = CheckWhetherToStartRecovery (m);
	...
	void UpdatePatient(Patient p) {
		...
		if (p.pulseRate > 0f) {
			...
			bool pulseAffected = false; 
			bool stillRequiringTreatment = false;


			foreach (Medication m in p.prescriptions) {
				...
				if (m.dose > 0f && m.dosesAdministered < 99) {
					...
					if (p.pulseRate > 0f) {
						...
						if (m.currentLevel <= 0f) {
							...
							//	p.pulseRate = p.pulseRate - pulseRateDecreaseFromOverdose * Time.deltaTime;
				...
				p.pulseRate = Mathf.Clamp (p.pulseRate, 0f, 160f);
			...
			if (stillRequiringTreatment == false) {
				...
				p.inTreatment = false;
				BedController.Instance.WheelBedOutOfBerth (p.bed);
				GameController.Instance.UpdateScore (5000);
				GameController.Instance.patientsCured++;
				GameController.Instance.curedText.text = "Cured: " + GameController.Instance.patientsCured.ToString();
				InAudio.Play (this.gameObject, SoundController.Instance.bye);
			...
			if (p.deathTimer >= 30f) {
				...
				if (p.isMale) {
					hud.boy.transform.DOLocalMove (new Vector3 (hud.boy.transform.position.x, 10f, hud.boy.transform.position.z), 2f);

				} else {

					hud.girl.transform.DOLocalMove (new Vector3 (hud.girl.transform.position.x, 10f, hud.girl.transform.position.z), 2f);
				}
		...
		foreach (Medication m in p.currentPrescriptions.Values) {
			...
			if (m.name == "Blood") {
				...
				if (m.dose > 0f) {
					...
					hud.bloodText.gameObject.SetActive (true);
					hud.bloodText.text = m.dose.ToString ();
				

				} else {

					hud.bloodText.gameObject.SetActive (false);
					hud.successText.gameObject.SetActive (false);
					//hud.traumaBar.sizeDelta = new Vector2 (0f, hud.traumaBar.sizeDelta.y);
	...
	public void SetupPatient(Patient p) {
		...
		hud.bloodText.gameObject.SetActive (false);
		hud.defibText.gameObject.SetActive (false);
	...
	public void AdministerTreatment(Patient p, float amountInjected, float maxAllowed, float injectionQuality, string medName) {
		...
		bool hadPrescription = false;
		bool causesBloodLoss = false;
		bool alreadyHaveBlood = false;
		...
		if (hadPrescription) {
			...
			Debug.Log ("Amount injected: " + amountInjected + " of " + maxAllowed);
			foreach (Medication m in p.prescriptions) {
				...
				if (m.name == medName) {
					...
					GameController.Instance.UpdateScore (Mathf.RoundToInt (injectionQuality * 100f));

					if (m.name != "Blood") {

						if (injectionQuality < 0.4f) {

							InAudio.Play (this.gameObject, SoundController.Instance.ouch);
							float choose = Random.Range (0f, 400f);

							if (choose < (100f - (injectionQuality * 100f))) {

								causesBloodLoss = true;
					...
					if (m.dose > 0f) {
						...
						if (m.currentLevel < 0f) {
							...
							p.pulseRate = p.pulseRate + 80f / m.dose * -m.currentLevel;
						...
						if (m.currentLevel > 0f) {
							...
							p.pulseRate = p.pulseRate + 30f / m.dose * m.currentLevel;
					...
					m.dosesAdministered = m.dosesAdministered + 1;
					Debug.Log (p.name + " has had " + m.dosesAdministered + " of " + m.dosesUntilCured);
			...
			p.pulseRate = p.pulseRate + 50f;
			hud.successText.text = "Wrong medication!!";


		}

		if (causesBloodLoss && alreadyHaveBlood == false) {

			Debug.Log ("Blood loss!!");

			GameObject bedGo2 = BedController.Instance.bedToGameObjectMap[p.bed];
			BedHud hud2 = bedGo2.GetComponent<BedHud> ();
	...
	void TriggerCardiacArrest(Patient p) {
		...
		hud.defibText.gameObject.SetActive (true);
	...
	public void UpdateBedHud(Patient p) {
